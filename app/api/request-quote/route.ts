...
async function loadFonts(pdf: PDFDocument) {
  ...
  // Safely read a TTF/OTF and validate magic bytes
  async function readFontSafe(absPath: string): Promise<Uint8Array | null> {
    try {
      ...
      const isTTF =
        (b0 === 0x00 && b1 === 0x01 && b2 === 0x00 && b3 === 0x00) || // 00010000
        (b0 === 0x74 && b1 === 0x72 && b2 === 0x75 && b3 === 0x65);     // "true"
      if (!(isOTF || isTTF)) return null;
      return bytes;
    } catch {
      return null;
    }
  ...
  const regBytes = await readFontSafe(regPath);
  const boldBytes = await readFontSafe(boldPath);

  try {
    if (regBytes) {
      const noto = await pdf.embedFont(regBytes, { subset: true });
      const notoB = boldBytes ? await pdf.embedFont(boldBytes, { subset: true }) : noto;
      return { noto, notoB };
    }
  } catch (e) {
    console.warn("Custom font embed failed; falling back to StandardFonts:", (e as Error)?.message);
  }

  // ✅ Fallback so the route never 500s if custom fonts are bad/missing
