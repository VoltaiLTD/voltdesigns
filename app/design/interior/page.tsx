...
export default function InteriorStepPage() {
  const r = useRouter();
  const items = useMemo(() => listByDesign("interior"), []);
  ...
  function onContinue() {
    if (!selected.length) {
      alert("Select one or more interior materials.");
      return;
    }
    r.push(`/ai-design-visualizer?type=interior&materials=${encodeURIComponent(selected.join(","))}`);
  }

  return (
    <main className="max-w-6xl mx-auto px-6 py-10">
      <h1 className="text-2xl font-semibold">Interior — choose materials</h1>
      <p className="text-sm text-gray-600 mt-1">
        Pick any combination (e.g., trims + panels). You can still tweak inside the visualizer.
      </p>
